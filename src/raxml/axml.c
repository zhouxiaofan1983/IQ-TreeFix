	      ...
	      for(i = 0; i < tr->NumberOfModels; i++)
		{
		  ...
		  if(modelStillExists)
		    {	  
		      ...
		      char *protModels[18] = {"DAYHOFF", "DCMUT", "JTT", "MTREV", "WAG", "RTREV", "CPREV", "VT", "BLOSUM62", "MTMAM", "LG", "MTART", "MTZOA", "PMB", "HIVB", "HIVW", "JTTDCMUT", "FLU"};
		      int k = 1;
		      int lower, upper;
		      int parts = 0;
  ...
  if(processID == 0)
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  ...
	  switch(tr->partitionData[model].dataType)
		...
		char *protStrings[18] = {"DAYHOFF", "DCMUT", "JTT", "MTREV", "WAG", "RTREV", "CPREV", "VT", "BLOSUM62", "MTMAM", "LG", "MTART", "MTZOA", "PMB", "HIVB", "HIVW", "JTTDCMUT", "FLU"};

		char basesPROT[20] = {'A', 'R', 'N' , 'D', 'C', 'Q','E','G','H','I','L','K','M','F','P','S','T','W','Y','V'};		

		assert(tr->partitionData[model].protModels >= 0 && tr->partitionData[model].protModels < 18);		

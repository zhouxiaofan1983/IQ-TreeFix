...
class CoarseModelSpeed(StatModel):
    ...
    def compute_lik_test(self, gtree, stat=None, alternative=None):
...
class CoarseModel(StatModel):
    ...
    def compute_lik_test(self, gtree, stat=None, alternative=None):
        ...
        if self.test == "AU":
            os.system('iqtree-omp -redo -nt %s -m %s -st %s -s %s -te %s -z %s.treefix_tmp.tree -zb %s -au -pre %s.treefix_tmp > /dev/null' % (self.cpu, self.model, self.type, self.seqfile, self.btreefile, self.pre, self.rep, self.pre))
        else:
            os.system('iqtree-omp -redo -nt %s -m %s -st %s -s %s -te %s -z %s.treefix_tmp.tree -zb %s -pre %s.treefix_tmp > /dev/null' % (self.cpu, self.model, self.type, self.seqfile, self.btreefile, self.pre, self.rep, self.pre))
    ...
    def optimize_model(self, gtree, aln):
        ...
        if self.type == "DNA" or "+G" in self.model:
            ...
            if self.type == "DNA":
                ...
                # set command for stats
                if "+G" in self.model:
                    self.cmd = 'iqtree-omp -redo -nt %s -m %s+G+Fu -a %s -st %s -s %s' % (self.cpu, self.modelfile, self.gamma, self.type, self.seqfile)
                else:
                    self.cmd = 'iqtree-omp -redo -nt %s -m %s+Fu -st %s -s %s' % (self.cpu, self.modelfile, self.type, self.seqfile)
            # other data type
            elif "+G" in self.model:
                # read estimated model parameters from IQTREE outptut
                fi = open("%s.treefix_tmp.iqtree" % self.pre, 'r')
                ...
                # set command for stats
                self.cmd = 'iqtree-omp -redo -nt %s -m %s -a %s -st %s -s %s' % (self.cpu, self.model, self.gamma, self.type, self.seqfile)
        else:
            # set command for stats
            self.cmd = 'iqtree-omp -redo -nt %s -m %s -st %s -s %s' % (self.cpu, self.model, self.type, self.seqfile)
...
class FineModel(StatModel):
    ...
    def compute_lik_test(self, gtree, stat=None, alternative=None):
        ...
        os.system('iqtree-omp -redo -nt %s -m %s -st %s -s %s -te %s -pre %s.treefix_tmp -wsl > /dev/null' % (self.cpu, self.model, self.type, self.seqfile, gtreefile, self.pre))
        fo = open("%s.sitelh" % self.pre, 'w')
        fo.write(self.bsitelh)
        fi = open("%s.treefix_tmp.sitelh" % self.pre, 'r')
        fi.readline()
